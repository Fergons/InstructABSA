...
class DatasetLoader:
    ...
    def reconstruct_strings_with_n_features(self, df, col, num_features=3):
        """
        Reconstruct strings to dictionaries when loading csv/xlsx files.
        """
        reconstructed_col = []
    ...
    def create_data_in_atsc_format(self, df, on, key, text_col, aspect_col, bos_instruction='',
        ...
        df['text'] = df[[text_col, aspect_col]].apply(
            lambda x: bos_instruction + x[0] + delim_instruction + x[1] + eos_instruction, axis=1)
    ...
    def create_data_in_joint_task_aspect_category_sentiment_extraction_format(
            self, df,
    ...
    def set_data_for_training_semeval(self, tokenize_function):
        ...
        if self.test_df_id is None:
            ...
            indomain_dataset = DatasetDict(
                {'train': Dataset.from_pandas(self.train_df_id), 'test': Dataset.from_pandas(self.test_df_id)})
        ...
        if (self.train_df_ood is not None) and (self.test_df_ood is None):
            ...
            other_domain_dataset = DatasetDict(
                {'train': Dataset.from_pandas(self.train_df_ood), 'test': Dataset.from_pandas(self.test_df_ood)})
            other_domain_tokenized_dataset = other_domain_dataset.map(tokenize_function, batched=True)
        else:
